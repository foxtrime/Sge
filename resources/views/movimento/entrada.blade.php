		   	...
		   	@if( isset($movimento))
				...
				<form id="frm_material" class="form-horizontal form-label-left needs-validation" method="post" action="{{ url("material/$material->id")}}" >
					<input type="hidden" id="material_id" class="form-control " name="material_id" value="{{$material->id}}"  >	
   					{!! method_field('PUT') !!}
			@else
				<form id="frm_material" class="form-horizontal form-label-left needs-validation" method="post" action="{{ url('entrada') }}" >
			@endif
				...
				<div class="form-group row">
					...
					{{-- <div class=" form-group col-md-2 col-sm-2 col-xs-12">
					   ...
					   <input type="text" id="qtd_min" class="form-control " name="qtd_min" value=0 disabled >	
				   </div> --}}

				 	<div class=" form-group col-md-2 col-sm-2 col-xs-12">
						<label class="control-label" >Quantidade da entrada</label>
						<input type="number" id="quantidade" class="form-control " name="quantidade"   
						value="{{ isset($movimento) ? $material->quantidade: old('$quantidade')}}" >	
					</div>
				...
				<div class="ln_solid"> </div>
					...
					<button id="btn_cancelar" class="botoes-acao btn btn-round btn-primary" 
							data-material = "{{isset($movimento) ? $material->material : old('material')}}" >
